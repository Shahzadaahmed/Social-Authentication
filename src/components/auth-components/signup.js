...
import { Button, Grid, Paper, TextField, Typography, FormControlLabel, Checkbox, makeStyles, Avatar } from '@material-ui/core';
import firebaseConfig from "../../config/firebase-config";
import firebase from "firebase";
import "./style.css";
import { signUpUser } from "../../store/action/auth-actions";
let pickProfileImageURL;
...
const SignUp = () => {
    ...
    // NoteProfile image onchange handler...!
    const onChangeImage = async (event) => {
        ...
        if (event.target.files && event.target.files[0]) {
            ...
            if (validImage) {
                ...
                // Note: Saving profile image to firebase storage...!
                let imageName = Date.now();
                let imagePath = await fetch(path);
                let blob = await imagePath.blob();
                let imageRef = firebase.storage().ref().child(`Profile-Images/${imageName}`);
                let response = await imageRef.put(blob);
                let getURL = await imageRef.getDownloadURL(); // Note: downloadURL from firebase storage recieved...!
                pickProfileImageURL = getURL;
                // console.log(pickProfileImageURL);
            }
    ...
    const onSubmit = () => {
        ...
        if (profileImage != null) {
            if (
                ...
                formData.userProfileImage = pickProfileImageURL
